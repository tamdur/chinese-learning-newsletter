...
def fetch_ctext_chapter(chapter_id: str) -> list[str]:
    """Fetch a Mengzi chapter from the ctext.org API.

    Uses the correct endpoint: https://api.ctext.org/gettext?urn=ctp:mengzi/{chapter_id}
    Returns a list of paragraph strings (Chinese text only — ctext API
    does not serve English translations).
    """
    url = f"https://api.ctext.org/gettext?urn=ctp:mengzi/{chapter_id}"
    print(f"    GET {url}")
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            body = resp.read()
            print(f"    → {resp.status} | Content-Type: {content_type} | {len(body)} bytes")

            if "application/json" not in content_type:
                print(f"    ERROR: Expected JSON, got {content_type}. First 200 chars:", file=sys.stderr)
            ...
            if "error" in data:
                print(f"    ERROR: API returned error: {data['error']}", file=sys.stderr)
                return []

            paragraphs = data.get("fulltext", [])
            title = data.get("title", "?")
            print(f"    → Title: {title} | {len(paragraphs)} paragraphs")
            return paragraphs
...
def build_mengzi():
    ...
    The API returns Chinese text only (no English translations).
    English translations will be generated by the translator-classical
    agent when the wisdom pipeline runs.
    """
    all_passages = []
    passage_index = 0
    chapters_fetched = 0
    ...
    for chapter_id, chapter_name in MENGZI_CHAPTERS:
        ...
        chapters_fetched += 1
        total_chars_in_chapter = sum(len(p) for p in paragraphs)
        print(f"    Total chars in chapter: {total_chars_in_chapter}")

        # Group short adjacent paragraphs into passages of ~150 chars
        current_text = ""

        for para_text in paragraphs:
            if current_text and len(current_text) + len(para_text) > 200:
    ...
    data = {
        ...
        "title": "孟子",
        "title_en": "Mencius",
        "note": "Chinese text only. English translations are generated by the translator-classical agent at pipeline runtime.",
    ...
    print(f"\n  Mengzi: {len(all_passages)} passages across {chapters_fetched}/{len(MENGZI_CHAPTERS)} chapters")
    print(f"  Written to: {output_path}")
    if chapters_fetched < len(MENGZI_CHAPTERS):
        print(f"  WARNING: {len(MENGZI_CHAPTERS) - chapters_fetched} chapters failed to fetch", file=sys.stderr)
    return data
